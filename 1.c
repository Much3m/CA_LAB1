...
void dec_to_bin(unsigned int n, int count, int *ptr) {
	if (count != 0)
		dec_to_bin(n/2,count -1,ptr);
	ptr[count] = n%2;
}

void print_inst(int *ptr, int size) {
...
void parse_instructions(void) {
	...
	while (running) {
		...
		int *inst = (int *)malloc(sizeof(int)*32);
